var stage1={
	function preload(){
		...
		this.load.spritesheet('firework','assets/firework.png',16,16);
		...
		this.load.tilemap('level', 'assets/2016096.json', null,Phaser.Tilemap.TILED_JSON);
		game.load.audio('theme','audio/Super-Mario-Bros-Theme-Song.ogg');
	...
	function create(){
		...
		map.createFromTiles(9, null, 'firework', 'stuff', fireworks);
		fireworks.callAll('animations.add', 'animations', 'spin',[ 0, 1, 2], 3, true);
		fireworks.callAll('animations.play','animations','spin');
	...
	function update(){
		...
		game.physics.arcade.overlap(player, fireworks, fireworkOverlap);
		game.physics.arcade.overlap(player, teleporters, teleporterOverlap);
		var jsound=game.add.audio('jump');
	...
	function teleporterOverlap(player,teleporter){
		...
		if(cursors.up.isDown)
			player.reset(1395,195);
		if(cursors.down.isDown)
			player.reset(1095,165);
	}
}
